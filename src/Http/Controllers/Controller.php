        ...
        switch ($row->type) {
            ...
            case 'file':
                if ($file = $request->file($row->field)) {
                    ...
                    $request->file($row->field)->storeAs(config('codeforlife.storage.subfolder').$path, $filename.'.'.$file->getClientOriginalExtension());
            ...
            case 'image':
                if ($request->hasFile($row->field)) {
                    ...
                    Storage::put(config('codeforlife.storage.subfolder').$fullPath, (string) $image, 'public');
                    ...
                    if (isset($options->thumbnails)) {
                        foreach ($options->thumbnails as $thumbnails) {
                            ...
                            Storage::put(
                                config('codeforlife.storage.subfolder').$path.$filename.'-'.$thumbnails->name.'.'.$file->getClientOriginalExtension(),
                                (string) $image, 'public'
                            );
                        }
                    }
